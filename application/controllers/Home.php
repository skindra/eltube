...
class Home extends MY_Controller {
	...
	{
		$baru_upload = $this->Home->query('select * from david order by date_upload desc limit 12')->result();
        $video = $this->Home->query('select * from david ORDER BY id DESC limit 12')->result();
        $viewer = $this->Home->query('SELECT * FROM david ORDER BY RAND() LIMIT 12')->result();
		$judul = "Menu Utama";
